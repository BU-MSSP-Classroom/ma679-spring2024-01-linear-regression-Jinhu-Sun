...
lmfit_2008<- lm(log(SalePrice) ~ `Year Built`+`Year Remod/Add`+`Gr Liv Area`+`Full Bath`+`Half Bath`+`Bedroom AbvGr`+ `Garage Area`, data = ames_raw_2008)# ["your model here"] # use ames_raw_2008
summary(lmfit_2008)
```
...
It is a ok fit since my fit has a R square of 76.96%.
...
~~~
Please write your answer in full sentences.
Definitely not because they will cause overfitting. And our loss function calculator puts more punishment on overfitting than underfitting.
...
Your code:
```{r,echo=TRUE}
interaction <- lm(log(SalePrice) ~ (`Year Built`+`Year Remod/Add`+`Gr Liv Area`+`Full Bath`+`Half Bath`+`Bedroom AbvGr`+`Garage Area`)^2, data = ames_raw_2008)
summary(interaction)

interaction1 <- lm(log(SalePrice) ~ `Year Built`+`Year Remod/Add`+`Gr Liv Area`+`Full Bath`+`Half Bath`+`Bedroom AbvGr`+`Garage Area`+`Year Built`*`Half Bath`+`Gr Liv Area`*`Garage Area`, data = ames_raw_2008)
summary(interaction1)
pred_2009<-exp(predict(interaction1,newdata=ames_raw_2009))
calc_loss(pred_2009,ames_raw_2009$SalePrice)
```
...
~~~
Please write your answer in full sentences.
I choose Year Built`:`Half Bath` and `Gr Liv Area`:`Garage Area` as the interaction, as the result my loss function did have a little decrease.
...
Your code:
```{r,echo=TRUE}
# Define the formula with polynomial terms up to the 3rd order for each predictor
formula <- log(SalePrice) ~ poly(`Year Built`, 3) + poly(`Year Remod/Add`, 3) + 
           poly(`Gr Liv Area`, 3) + poly(`Full Bath`, 3) + poly(`Half Bath`, 3,raw=TRUE) + 
           poly(`Bedroom AbvGr`, 3) + poly(`TotRms AbvGrd`, 3) + poly(`Garage Area`, 3)

# Fit the model using the lm() function in R
lmfit_2008_poly <- lm(formula, data = ames_raw_2008)

# Summarize the model to see the significance of the polynomial terms
summary(lmfit_2008_poly)
...
Your code:
```{r,echo=TRUE}
ames_raw$predicted_price <- predict(interaction1, newdata = ames_raw)

ames_raw$residuals <- with(ames_raw, SalePrice - predicted_price)

ames_raw$over_priced <- with(ames_raw, SalePrice > predicted_price)

top_overpriced <- ames_raw[ames_raw$over_priced, ][order(-ames_raw$residuals), ][1:5, ]
...
Your code:
```{r,echo=TRUE}
ames_raw$residuals <- with(ames_raw, predicted_price - SalePrice)

ames_raw$good_deal <- with(ames_raw, SalePrice < predicted_price)

top_deals <- ames_raw[ames_raw$good_deal, ][order(ames_raw$residuals), ][1:5, ]
...
~~~
Please write your answer in full sentences.
According to my model, there is no good deal in the list.
...
~~~
Please write your answer in full sentences.
Yes, there is a relationship between the predictor and the response. The p-value for the horsepower coefficient is so small, which indicate a strong statistical significance.

The strength of the relationship is the R-squared value, which is 0.6059. This means that approximately 60.59% of the variability in mpg can be explained by horsepower. This is a strong relationship.

The relationship is negative. This means that as horsepower increases, the mpg tends to decrease.

mpg = Intercept + (Coefficient × horsepower) = 39.935861 + (-0.157845 × 98)
~~~
...
Your code:
```{r,echo=TRUE}
plot(Auto$horsepower, Auto$mpg, main="Regression of mpg on horsepower",
...
~~~
Please write your answer in full sentences.
The QQ plot is mostly a straight with some off at both ends suggesting the presence of outliers or that the normality assumption may not be completely valid. The pattern on the scale-location indicates potential heteroscedasticity.
...
auto_data_numeric <- select_if(auto_data, is.numeric)

pairs(auto_data_numeric)
```
...
correlation_matrix <- cor(auto_data_numeric)
...
Your code:
```{r,echo=TRUE}
auto_data_modified <- select(auto_data, -name)

model <- lm(mpg ~ ., data = auto_data_modified)
...
~~~
Please write your answer in full sentences.
The R-square is as high as 0.8215 which means the model is good for these data.
The coefficients for weight, year, and origin are significant.
This positive coefficient for year implies that cars have become more fuel-efficient over time during the period covered by the dataset.
...
~~~
Please write your answer in full sentences.
The QQ plot suggest that there might be some outliers.
...
Your code:
```{r,echo=TRUE}
model_interaction <- lm(mpg ~ weight*year, data = auto_data_modified)
...
~~~
Please write your answer in full sentences.
if you suspect the effect of weight on mpg depending on the year of the car, you would include an interaction term between weight and year, and by looking at the result weight:year are significant.
...
~~~
Please write your answer in full sentences.
The coefficient for 'Price' is approximately -0.0545, which means for each 1 unit increase in price, the sales decrease by 0.0545 units.

The coefficient for 'UrbanYes' is approximately -0.0219, which suggests that being in an urban location is associated with a decrease in sales by about 0.0219 units compared to non-urban locations.

Stores located in the US have sales that are about 1.2006 units higher, on average, than stores not located in the US
~~~
...
~~~
Please write your answer in full sentences.
Sales = Bo + B1*Price + B2*UrbanYes + B3*USYes + e
...
~~~
Please write your answer in full sentences.
we reject the null hypothesis that the coefficient for Price is zero since the p-value is so small

We cannot reject the null hypothesis for the UrbanYes coefficient

we reject the null hypothesis for the USYes coefficient
~~~
...
~~~
Please write your answer in full sentences.
No good at all, both of them have a R-square of 0.2393 which means those are really bad model.
